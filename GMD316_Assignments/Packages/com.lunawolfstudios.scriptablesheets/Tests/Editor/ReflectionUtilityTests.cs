...
namespace LunaWolfStudiosEditor.ScriptableSheets.EditorTests
{
	...
	public class ReflectionUtilityTests
	{
		...
		[Test]
		public void GetNestedFieldType_WhenInvalidFieldAndLogWarningFalse_ReturnsNullWithoutWarning()
		{
			var warnings = 0;
			...
			try
			{
				// Distinct path so the static type cache from other tests doesn't short circuit the lookup and mask the warning.
				var fieldType = ReflectionUtility.GetNestedFieldType(typeof(DerivedClass), "m_SilentInvalidPath", false);
				Assert.IsNull(fieldType);
			}
			finally
			{
