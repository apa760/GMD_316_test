...
namespace LunaWolfStudiosEditor.ScriptableSheets.EditorTests
{
	...
	public class SearchFilterTests
	{
		...
		[Test]
		public void GetObjects_ByNameProperty()
		{
			var result = SearchFilter.GetObjects("p:m_Name=MyName2", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(1, result.Count);
			Assert.AreEqual("MyName2", result[0].name);
		}

		[Test]
		public void GetObjects_ByNamePropertyOnComponent()
		{
			// Components do not expose m_Name as a serialized property, but the Name field still filters by the Object name.
			var gameObjects = new[] { new GameObject("CompAlpha"), new GameObject("CompBeta"), new GameObject("Other") };
			try
			{
				var components = new List<Object> { gameObjects[0].transform, gameObjects[1].transform, gameObjects[2].transform };

				var result = SearchFilter.GetObjects("p:m_Name=CompAlpha", components, m_DefaultSearchSettings, false, false);
				Assert.AreEqual(1, result.Count);
				Assert.AreEqual("CompAlpha", result[0].name);

				result = SearchFilter.GetObjects("p:m_Name~=Comp", components, m_DefaultSearchSettings, false, false);
		...
		[Test]
		public void GetObjects_ByChainedAnd()
		{
			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyInt>1 & m_MyRootObject.m_MyInt<4", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt > 1 && ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt < 4));
		}

		[Test]
		public void GetObjects_ByChainedAndAcrossProperties()
		{
			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyInt>=2 & m_MyRootObject.m_MyBool=true", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt >= 2 && ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyBool));
		}

		[Test]
		public void GetObjects_ByChainedOr()
		{
			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyInt=0 | m_MyRootObject.m_MyInt=4", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt == 0 || ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt == 4));
		}

		[Test]
		public void GetObjects_ByChainedAndOrPrecedence()
		{
			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyBool=true & m_MyRootObject.m_MyInt>2 | m_MyRootObject.m_MyInt=1", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj =>
			{
				var rootObject = ((TestUtility.TempScriptableObject) obj).m_MyRootObject;
				return (rootObject.m_MyBool && rootObject.m_MyInt > 2) || rootObject.m_MyInt == 1;
			}));
		}

		[Test]
		public void GetObjects_ByChainedAndWithContains()
		{
			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyString~=MyString & m_MyRootObject.m_MyInt>2", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(2, result.Count);
		}

		[Test]
		public void GetObjects_LiteralAmpersandInValueIsNotChained()
		{
			var tempScriptableObject = (TestUtility.TempScriptableObject) m_TempObjects[0];
			tempScriptableObject.m_MyRootObject.m_MyString = "this&that";

			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyString=this&that", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(1, result.Count);
			Assert.AreEqual("this&that", ((TestUtility.TempScriptableObject) result[0]).m_MyRootObject.m_MyString);
		}

		[Test]
		public void GetObjects_LiteralPipeInValueIsNotChained()
		{
			var tempScriptableObject = (TestUtility.TempScriptableObject) m_TempObjects[1];
			tempScriptableObject.m_MyRootObject.m_MyString = "a|b";

			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyString=a|b", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(1, result.Count);
			Assert.AreEqual("a|b", ((TestUtility.TempScriptableObject) result[0]).m_MyRootObject.m_MyString);
		}

		[Test]
		public void GetObjects_ChainedInvalidConditionLeavesInputUnfiltered()
		{
			var result = SearchFilter.GetObjects("p:m_MyRootObject.m_MyInt>1 & invalidPath<4", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(m_TempObjects.Count, result.Count);
		}

		// Maps column indices to property paths so $ references can resolve. Index 0 mirrors the Actions column.
		private static readonly string[] s_ColumnPropertyPaths =
		{
			Layout.ColumnUtility.ActionsTooltip,
			"m_Name",
			"m_MyRootObject.m_MyInt",
			"m_MyRootObject.m_MyBool",
			"m_MyRootObject.m_MyString",
		};

		[Test]
		public void GetObjects_ByColumnIndexAlpha()
		{
			var result = SearchFilter.GetObjects("p:$C>2", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt > 2));
		}

		[Test]
		public void GetObjects_ByColumnIndexAlphaIsCaseInsensitive()
		{
			var result = SearchFilter.GetObjects("p:$c>2", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(2, result.Count);
		}
		...
		[Test]
		public void GetObjects_ByColumnIndexNumeric()
		{
			var result = SearchFilter.GetObjects("p:$2>2", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt > 2));
		}

		[Test]
		public void GetObjects_ByChainedColumnIndex()
		{
			var result = SearchFilter.GetObjects("p:$2>1 & $C<4", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt > 1 && ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt < 4));
		}

		[Test]
		public void GetObjects_ByChainedColumnIndexAcrossProperties()
		{
			var result = SearchFilter.GetObjects("p:$2>=2 & $D=true", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(2, result.Count);
			Assert.IsTrue(result.All(obj => ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyInt >= 2 && ((TestUtility.TempScriptableObject) obj).m_MyRootObject.m_MyBool));
		}

		[Test]
		public void GetObjects_ColumnIndexWithoutPathsLeavesInputUnfiltered()
		{
			var result = SearchFilter.GetObjects("p:$2>2", m_TempObjects, m_DefaultSearchSettings, false, false);
			Assert.AreEqual(m_TempObjects.Count, result.Count);
		}

		[Test]
		public void GetObjects_ColumnIndexOutOfRangeLeavesInputUnfiltered()
		{
			var result = SearchFilter.GetObjects("p:$99>0", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(m_TempObjects.Count, result.Count);
		}

		[Test]
		public void GetObjects_ColumnIndexNonPropertyColumnLeavesInputUnfiltered()
		{
			var result = SearchFilter.GetObjects("p:$A>0", m_TempObjects, m_DefaultSearchSettings, false, false, s_ColumnPropertyPaths);
			Assert.AreEqual(m_TempObjects.Count, result.Count);
		}
	}
}
