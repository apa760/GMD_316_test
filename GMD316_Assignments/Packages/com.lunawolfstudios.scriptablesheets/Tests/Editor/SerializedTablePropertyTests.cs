...
namespace LunaWolfStudiosEditor.ScriptableSheets.EditorTests
{
	...
	public class SerializedTablePropertyTests
	{
		...
		[Test]
		public void SetProperty_StripsPercentage_WhenSettingIntValue()
		{
			m_MyIntProperty.SetProperty("3%", m_FormatSettings);
			Assert.AreEqual(3, m_MyScriptableObject.myInt);

			m_MyByteProperty.SetProperty("7%", m_FormatSettings);
			Assert.AreEqual(7, m_MyScriptableObject.myByte);

			m_MyShortProperty.SetProperty("12%", m_FormatSettings);
			Assert.AreEqual(12, m_MyScriptableObject.myShort);

			m_MyLongProperty.SetProperty("50%", m_FormatSettings);
			Assert.AreEqual(50, m_MyScriptableObject.myLong);

#if UNITY_2022_1_OR_NEWER
			m_MyUIntProperty.SetProperty("9%", m_FormatSettings);
			Assert.AreEqual(9u, m_MyScriptableObject.myUInt);

			m_MyULongProperty.SetProperty("100%", m_FormatSettings);
			Assert.AreEqual(100ul, m_MyScriptableObject.myULong);
#endif
		}
		...
		[Test]
		public void SetProperty_DividesPercentageBy100_WhenSettingFloatValue()
		{
			m_MyFloatProperty.SetProperty("3%", m_FormatSettings);
			Assert.AreEqual(0.03f, m_MyScriptableObject.myFloat, 0.0001f);

			m_MyFloatProperty.SetProperty("50%", m_FormatSettings);
			Assert.AreEqual(0.5f, m_MyScriptableObject.myFloat);

			m_MyDoubleProperty.SetProperty("25%", m_FormatSettings);
			Assert.AreEqual(0.25d, m_MyScriptableObject.myDouble);
		}
