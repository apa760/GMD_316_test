...
namespace LunaWolfStudiosEditor.ScriptableSheets.Tables
{
	public struct SerializedTableProperty : ITableProperty
	{
		...
		public string GetProperty(FlatFileFormatSettings formatSettings)
		{
			...
			if (property == null)
			{
				// The field belongs to a managed reference subtype this Object isn't currently using.
				return string.Empty;
			}
		...
		public void SetProperty(string value, FlatFileFormatSettings formatSettings)
		{
			...
			if (property != null)
			{
				if ((!property.editable && property.propertyPath != UnityConstants.Field.Name) || value == null)
				{
				...
				switch (property.propertyType)
				{
					...
					case SerializedPropertyType.String:
						// Only the top level asset name property renames the Object.
						if (property.propertyPath == UnityConstants.Field.Name)
						{
			...
			{
				// A missing array element under a serialize reference field is expected when the row's subtype declares a
				// different array size, so let the managed reference branch below skip it silently instead of warning.
				if (SerializedPropertyUtility.IsArrayElement(m_PropertyPath) && !HasManagedReferenceAncestor(serializedObject))
				{
				...
				else if (HasManagedReferenceAncestor(serializedObject))
				{
					// The field belongs to a managed reference subtype this Object isn't currently using, so there is nothing to set.
				}
				else
				{
		...
		public void AddNewLine()
		{
			var serializedObject = GetSerializedObject();
			var property = GetSerializedProperty(serializedObject);
			if (property == null || property.propertyPath == UnityConstants.Field.Name)
			{
				return;
			}
		...
		{
			var property = GetSerializedProperty();
			return property != null && property.IsInputFieldProperty(isScriptableObject);
		}

		// True when an ancestor segment of the property path is a serialize reference field, which means a missing leaf
		// is expected for polymorphic data (the Object's current subtype simply doesn't declare that field).
		private bool HasManagedReferenceAncestor(SerializedObject serializedObject)
		{
			var separatorIndex = m_PropertyPath.LastIndexOf('.');
			while (separatorIndex > 0)
			{
				var ancestorPath = m_PropertyPath.Substring(0, separatorIndex);
				var ancestor = serializedObject.FindProperty(ancestorPath);
				if (ancestor != null && ancestor.IsManagedReference())
				{
					return true;
				}
		...
		public bool NeedsSelectionBorder(bool lockNames = false)
		{
			...
			// Asset references focus a zero-size proxy control, so draw the border ourselves to show the selected cell.
			return (!isNameProperty && property.IsReadOnly() || (isNameProperty && lockNames)) || property.propertyType == SerializedPropertyType.AnimationCurve || property.propertyType == SerializedPropertyType.Gradient || property.IsAssetReference();
		}
