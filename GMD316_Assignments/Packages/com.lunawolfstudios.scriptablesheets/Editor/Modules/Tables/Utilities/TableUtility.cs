...
namespace LunaWolfStudiosEditor.ScriptableSheets.Tables
{
	...
	{
		// Strips the header row (when present) and splits the content into individual rows, handling wrapping and the empty row option.
		private static string[] SplitRows(string flatFileContent, ref FlatFileFormatSettings formatSettings, out Wrapper wrapper)
		{
			wrapper = formatSettings.GetWrapper();
			if (formatSettings.HasHeaders)
			{
				var firstRowEndIndex = flatFileContent.IndexOf(formatSettings.RowDelimiter);
				var joinedColumnHeadersSanitized = SanitizeHeader(formatSettings.GetJoinedColumnHeaders(wrapper));
				if (firstRowEndIndex >= 0)
				{
					var headerRow = flatFileContent.Substring(0, firstRowEndIndex).Trim();
					var dataContent = flatFileContent.Substring(firstRowEndIndex + formatSettings.RowDelimiter.Length);
					var headerRowSanitized = SanitizeIncomingHeader(headerRow, formatSettings);
					// Strip the header row when always stripping, when it matches the column headers, or when it is shaped like a header row.
					if (formatSettings.AlwaysStrip
						|| joinedColumnHeadersSanitized.Contains(headerRowSanitized) || headerRowSanitized.Contains(joinedColumnHeadersSanitized)
						|| LooksLikeHeaderRow(headerRow, dataContent, formatSettings, wrapper))
					{
						flatFileContent = dataContent;
					}
				}
				else
				{
					// Handle case where there's only a single row.
					var headerRowSanitized = SanitizeIncomingHeader(flatFileContent, formatSettings);
					if (formatSettings.AlwaysStrip || joinedColumnHeadersSanitized.Contains(headerRowSanitized))
					{
			...
			if (formatSettings.HasWrapping)
			{
				...
				var wrapperClose = wrapper.Close;
				var totalWrapChars = flatFileContent.Count(c => c == wrapperOpen || c == wrapperClose);
		...
		/// <summary>
		/// Counts the number of data rows in flat file content, excluding the header row when present.
		/// </summary>
		public static int GetFlatFileRowCount(string flatFileContent, FlatFileFormatSettings formatSettings)
		{
			if (string.IsNullOrEmpty(flatFileContent))
			{
				return 0;
			}
			return SplitRows(flatFileContent, ref formatSettings, out _).Length;
		}

		/// <summary>
		/// Counts the number of rows in JSON content. The top level keys represent rows in both the Flat and Hierarchy formats.
		/// </summary>
		public static int GetJsonRowCount(string json)
		{
			...
			try
			{
				var rowData = JsonConvert.DeserializeObject<Dictionary<string, object>>(json);
		...
		{
			var rowData = SplitRows(flatFileContent, ref formatSettings, out var wrapper);
			var rowIndex = 0;
		...
		private static string SanitizeIncomingHeader(string headerRow, FlatFileFormatSettings formatSettings)
		{
			if (formatSettings.HeaderColumnIndex && !formatSettings.HasWrapping)
			{
				var cells = headerRow.Split(new string[] { formatSettings.ColumnDelimiter }, StringSplitOptions.None);
				for (var i = 0; i < cells.Length; i++)
				{
					cells[i] = RemoveColumnIndexPrefix(cells[i].Trim());
				}
				headerRow = string.Join(formatSettings.ColumnDelimiter, cells);
			}
			return SanitizeHeader(headerRow);
		}

		private static string RemoveColumnIndexPrefix(string cell)
		{
			var spaceIndex = cell.IndexOf(' ');
			return spaceIndex >= 0 ? cell.Substring(spaceIndex + 1) : cell;
		}

		private static bool LooksLikeHeaderRow(string headerRow, string dataContent, FlatFileFormatSettings formatSettings, Wrapper wrapper)
		{
			var expectedColumnCount = formatSettings.ColumnHeaders.Length - formatSettings.FirstColumnIndex;
			if (expectedColumnCount <= 0 || string.IsNullOrEmpty(dataContent))
			{
				return false;
			}
			var headerCells = SplitColumns(headerRow, formatSettings.ColumnDelimiter, wrapper);
			if (headerCells.Length != expectedColumnCount || headerCells.Any(c => c.Length == 0 || IsNumericCell(c)))
			{
				return false;
			}
			var dataRows = dataContent.Split(new string[] { formatSettings.RowDelimiter }, StringSplitOptions.RemoveEmptyEntries);
			return dataRows.Any(row => SplitColumns(row, formatSettings.ColumnDelimiter, wrapper).Any(IsNumericCell));
		}

		private static string[] SplitColumns(string row, string columnDelimiter, Wrapper wrapper)
		{
			var cells = row.Split(new string[] { columnDelimiter }, StringSplitOptions.None);
			for (var i = 0; i < cells.Length; i++)
			{
				var value = cells[i].Trim();
				if (wrapper != null && value.Length >= 2 && value[0] == wrapper.Open && value[value.Length - 1] == wrapper.Close)
				{
		...
		private static bool IsNumericCell(string cell)
		{
			return double.TryParse(cell, NumberStyles.Any, CultureInfo.InvariantCulture, out _);
		}
	}
}
