	...
	public static class MultiColumnHeaderUtility
	{
		...
		public static List<TObject> GetSorted<TObject>(this MultiColumnHeader multiColumnHeader, List<TObject> unsorted) where TObject : Object
		{
			...
			if (sortedColumnIndex >= 0 && unsorted.Count > 1)
			{
				...
				else if (column.headerContent.tooltip == ColumnUtility.GroupNameTooltip)
				{
					// Collection Tables group rows by their owning Object, so the Group Name column sorts owners by name.
					var alphanumComparer = new AlphanumComparer();
					var ordered = isAscending ? unsorted.OrderBy(u => u != null ? u.name : string.Empty, alphanumComparer) : unsorted.OrderByDescending(u => u != null ? u.name : string.Empty, alphanumComparer);
					return ordered.ToList();
				}
				else if (column.headerContent.tooltip == ColumnUtility.CollectionIndexTooltip)
				{
					// The Index column orders element rows within each owner group, which is applied after grouping, not here.
