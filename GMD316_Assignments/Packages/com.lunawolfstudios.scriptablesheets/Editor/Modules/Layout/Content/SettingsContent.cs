...
namespace LunaWolfStudiosEditor.ScriptableSheets.Layout
{
	public class SettingsContent : Content
	{
		...
		public static class DigitField
		{
			...
			public static readonly GUIContent AssetPreviewDepth = new GUIContent("Asset Preview Depth", "How many levels of child ScriptableObject references the column header's <b>Map Asset Preview</b> menu will explore when listing object reference properties whose preview can be shown.");
			public static readonly GUIContent CollectionTableDepth = new GUIContent("Collection Table Depth", "How many levels deep the <b>Collection Table</b> dropdown explores when discovering array or list fields.\n\nHigher values descend further through serializable class fields.");
		...
		{
			public static readonly GUIContent ColumnIndexFormat = new GUIContent("Column Index Format", "The format used to label the column index.\n\n<b>Alpha</b> uses spreadsheet style letters (A, B, C, ... AA, AB).\n\n<b>Numeric</b> uses the zero based column number.");
			...
			public static readonly GUIContent RowIndexFormat = new GUIContent("Row Index Format", "The format used to label the row index.\n\n<b>Alpha</b> uses spreadsheet style letters (A, B, C, ... AA, AB).\n\n<b>Numeric</b> uses the zero based row number.");
		...
		{
			public static readonly GUIContent AlwaysStrip = new GUIContent("Always Strip", "Always discard the first row on import, regardless of whether it matches the header names.\n\nUseful when the incoming column names differ from the table's, or when changing the header format option frequently.\n\nDisable to only strip the first row when it matches the header names, which provides stricter data validation.");
			public static readonly GUIContent AutoGenerate = new GUIContent("Auto Generate", "Automatically create new ScriptableObjects when importing or pasting more rows than there are existing Objects.\n\nGenerates enough Objects to match the row count using the new Object naming and path settings, then parses the data. Only applies to creatable ScriptableObject types.");
			...
			public static readonly GUIContent CollectionTables = new GUIContent("Collection Tables", "Enable a toolbar dropdown to expand a single array or list field into a table of rows, grouped by their owning Object, instead of displaying its elements as columns.\n\nEach element becomes a row and the element's fields become the columns. Select <b>None</b> from the dropdown to keep the default column behavior.");
			...
			public static readonly GUIContent ConfirmGeneration = new GUIContent("Confirm Generation", "Display a confirmation message before auto generating new Objects, showing how many will be created.");
			...
			public static readonly GUIContent ScanHiddenSubAssets = new GUIContent("Hidden Sub Assets", "Scan hidden sub-assets that aren't shown under their main asset in the Project window.\n\nWhen disabled only visible sub-assets are scanned. When enabled hidden sub-assets are included, but any with a missing script will log a Unity warning that can't be suppressed (for example a URP Volume Profile's Volume Components).");
			...
			public static readonly GUIContent ManagedReferences = new GUIContent("Managed References", "Display columns for the fields of every concrete subtype assigned to a serialize reference field, along with a per-row dropdown to view or change each Object's concrete type.\n\nChanging the type requires Unity 2021.2 or newer.");
