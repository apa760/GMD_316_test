...
namespace LunaWolfStudiosEditor.ScriptableSheets.Shared
{
	public static class SerializedPropertyUtility
	{
		...
		// Cache of concrete, instantiable subtypes assignable to a serialize reference field, keyed by the field's declared base type.
		private static readonly Dictionary<Type, Type[]> s_ConcreteManagedReferenceTypes = new Dictionary<Type, Type[]>();
		...
		private static readonly GUIContent s_PlayButtonContent = EditorGUIUtility.IconContent("PlayButton");
		private static readonly GUIContent s_PlayButtonOnContent = EditorGUIUtility.IconContent("PlayButton On");
		...
		// Draws an Object field with a trailing play button when the reference is an AudioClip. Returns false when it is not an AudioClip so the caller can draw normally.
		private static bool TryDrawAudioClipField(Rect propertyRect, SerializedProperty property, Type objectType, Action onResetTextEditing)
		{
			...
			var playRect = new Rect(objectRect.xMax, propertyRect.y, InlineButtonWidth, propertyRect.height);
			if (objectType != null)
			{
			...
			{
				EditorGUI.PropertyField(objectRect, property, GUIContent.none, false);
			}
			var isPlaying = AudioClipUtility.IsPlaying(audioClip);
			var playContent = isPlaying ? s_PlayButtonOnContent : s_PlayButtonContent;
			playContent.tooltip = $"{(isPlaying ? "Stop" : "Play")} {audioClip.name}";
		...
		public static void DrawProperty(this SerializedProperty property, Rect propertyRect, Object rootObject, bool isCustomField, out bool arraySizeChanged, bool showReadOnly, AssetPreviewSettings previewSettings, string defaultNewAssetPath, Action<ScriptableObject, Type> onObjectCreated, Action onResetTextEditing, Object previewOverride = null, string controlName = null)
		{
			arraySizeChanged = false;
			// A mapped preview source overrides this cell's preview for any property type. Drawing it here and shrinking the rect
			// keeps the field on the first line and suppresses the default object reference preview drawn further below.
			if (previewOverride != null && previewSettings.Show && propertyRect.height > EditorGUIUtility.singleLineHeight)
			{
				DrawUtility.TableAssetPreview(previewOverride, propertyRect, previewSettings);
			...
			if (isCustomField)
			{
				...
				switch (propertyType)
				{
					...
					case SerializedPropertyType.ObjectReference:
						...
						if (!IsDefaultUnityEngineType(rootObject))
						{
							...
							if (objectType != null)
							{
								...
								// If the Object is an AudioClip then give the option to play a preview of it directly from here.
								else if (!TryDrawAudioClipField(propertyRect, property, objectType, onResetTextEditing))
								{
									property.objectReferenceValue = EditorGUI.ObjectField(propertyRect, property.objectReferenceValue, objectType, false);
								}
							}
							else if (!TryDrawAudioClipField(propertyRect, property, null, onResetTextEditing))
							{
					...
					default:
						// Only draw asset previews if the height is greater than 1.
						if (property.IsAssetReference() && propertyRect.height > EditorGUIUtility.singleLineHeight)
						{
							DrawUtility.TableAssetPreview(property.GetAssetReferenceObject(), propertyRect, previewSettings);
						}
						propertyRect.height = EditorGUIUtility.singleLineHeight;
						var convertedAssetReferenceEnter = false;
						if (property.IsAssetReference())
						{
							// Our proxy is focused, not the drawer's control, so convert Enter into a synthetic picker click that
							// opens the selector through the drawer's normal mouse path. Done before the proxy consumes the Return.
							var assetEvent = Event.current;
							if (!string.IsNullOrEmpty(controlName)
								&& assetEvent.type == EventType.KeyDown
								&& (assetEvent.keyCode == KeyCode.Return || assetEvent.keyCode == KeyCode.KeypadEnter)
								&& GUI.GetNameOfFocusedControl() == controlName)
							{
								assetEvent.type = EventType.MouseDown;
								assetEvent.button = 0;
								assetEvent.mousePosition = new Vector2(propertyRect.xMax - 8f, propertyRect.y + EditorGUIUtility.singleLineHeight * 0.5f);
								convertedAssetReferenceEnter = true;
							}
							// The drawer assigns its own control name, so consume our pending one with a zero-size focusable proxy that
							// keyboard nav can land on. The visible drawer still handles mouse interaction and editing.
							EditorGUI.DropdownButton(new Rect(propertyRect.x, propertyRect.y, 0f, 0f), GUIContent.none, FocusType.Keyboard, GUIStyle.none);
						}
						EditorGUI.PropertyField(propertyRect, property, GUIContent.none, false);
						if (convertedAssetReferenceEnter)
						{
							// Consume the synthetic click if the drawer did not, so it does not leak to other cells.
							if (Event.current.type == EventType.MouseDown)
							{
								Event.current.Use();
							}
							// The drawer focused its own control, so restore our proxy to keep keyboard nav working afterward.
		...
		// Returns the array or list field paths that can be expanded into Collection Tables, descending through serializable class
		// fields up to maxDepth segments deep. Collections nested inside another collection's elements are skipped since they
		// cannot resolve to a single table without choosing a parent element.
		public static List<string> GetCollectionFieldPaths(this SerializedObject serializedObject, int maxIterations, int maxDepth)
		{
			...
			var iterations = 0;
			while (iterator.NextVisible(enterChildren) && iterations++ < maxIterations)
			{
				enterChildren = false;
				if (iterator.propertyPath == UnityConstants.Field.Name || iterator.propertyPath == UnityConstants.Field.Script)
				{
					continue;
				}
				...
				else if (iterator.propertyType == SerializedPropertyType.Generic && iterator.hasVisibleChildren && !iterator.IsAssetReference())
				{
					// Descend into a serializable class field so its array or list fields are offered too. The class field sits at
					// depth 1, so its children are evaluated at depth 2.
					AddNestedCollectionFieldPaths(iterator, paths, maxIterations, maxDepth, 2);
				}
			}
			return paths;
		}

		private static void AddNestedCollectionFieldPaths(SerializedProperty classProperty, List<string> paths, int maxIterations, int maxDepth, int depth)
		{
			if (depth > maxDepth)
			{
				return;
			}
			var child = classProperty.Copy();
			var end = classProperty.GetEndProperty();
			var childPrefix = $"{classProperty.propertyPath}.";
			var enterChildren = true;
			var iterations = 0;
			while (child.NextVisible(enterChildren) && !SerializedProperty.EqualContents(child, end) && iterations++ < maxIterations)
			{
				// Only inspect the direct children. Deeper nesting is handled by recursing into serializable class fields.
				enterChildren = false;
				if (!child.propertyPath.StartsWith(childPrefix))
				{
					break;
				}
				if (child.isArray && child.propertyType != SerializedPropertyType.String)
				{
					paths.Add(child.propertyPath);
				}
				else if (child.propertyType == SerializedPropertyType.Generic && child.hasVisibleChildren && !child.IsAssetReference())
				{
					AddNestedCollectionFieldPaths(child, paths, maxIterations, maxDepth, depth + 1);
				}
			}
		}
		...
		// Resolve the underlying asset an AssetReference points to so we can display its preview.
		public static Object GetAssetReferenceObject(this SerializedProperty property)
		{
			...
			if (!string.IsNullOrEmpty(subObjectName))
			{
				...
				var subObject = subAssets.FirstOrDefault(s => s.name == subObjectName);
				if (subObject != null)
				{
					return subObject;
				}
		...
		// True when the running Unity version can change a serialize reference field's concrete type via the public API.
		public static bool CanSetManagedReferenceValue =>
		...
		// Resolves the declared base type of a serialize reference field so we can enumerate its concrete subtypes.
		public static Type GetManagedReferenceFieldType(this SerializedProperty property, Object rootObject)
		{
			if (rootObject == null)
			{
				return null;
			}
			return GetManagedReferenceFieldType(property, rootObject.GetType());
		}

		public static Type GetManagedReferenceFieldType(this SerializedProperty property, Type type)
		{
			return ReflectionUtility.GetNestedFieldType(type, property.propertyPath);
		}

		// Returns the concrete, instantiable subtypes that can be assigned to a serialize reference field, cached per base type.
		public static Type[] GetConcreteManagedReferenceTypes(Type baseType)
		{
			if (baseType == null)
			{
				return Array.Empty<Type>();
			}
			if (!s_ConcreteManagedReferenceTypes.TryGetValue(baseType, out var types))
			{
				var derived = TypeCache.GetTypesDerivedFrom(baseType).Where(IsValidManagedReferenceType).OrderBy(t => t.Name);
				types = IsValidManagedReferenceType(baseType) ? new[] { baseType }.Concat(derived).ToArray() : derived.ToArray();
				s_ConcreteManagedReferenceTypes[baseType] = types;
			}
			return types;
		}

		private static bool IsValidManagedReferenceType(Type type)
		{
			return type != null && !type.IsAbstract && !type.IsGenericTypeDefinition && !typeof(Object).IsAssignableFrom(type) && type.GetConstructor(Type.EmptyTypes) != null;
		}

		// Short concrete type name of the currently assigned managed reference, or empty when null.
		public static string GetManagedReferenceTypeName(this SerializedProperty property)
		{
			return GetManagedReferenceTypeName(property.managedReferenceFullTypename);
		}

		public static string GetManagedReferenceTypeName(string managedReferenceFullTypename)
		{
			if (string.IsNullOrEmpty(managedReferenceFullTypename))
			{
				return string.Empty;
			}
			// Format is "<assembly> <namespace>.<type>"; reduce to the short type name.
			var typeName = managedReferenceFullTypename;
			var spaceIndex = typeName.IndexOf(' ');
			if (spaceIndex >= 0)
			{
				typeName = typeName.Substring(spaceIndex + 1);
			}
		...
		// Assigns a new instance of the given concrete type (or null) to a serialize reference field. Requires Unity 2021.2+.
		public static bool SetManagedReferenceValue(this SerializedProperty property, Type type)
		{
#if UNITY_2021_2_OR_NEWER
		...
		// Replaces a serialize reference value with a deep copy holding the same data. Duplicating an array element copies the
		// managed reference id, so both elements share one instance. Reassigning a fresh instance gives the copy its own id.
		public static void ReassignManagedReferenceValue(this SerializedProperty property)
		{
#if UNITY_2021_2_OR_NEWER
