		...
		public void ScanObjects(ScanSettings settings, SheetAsset sheetAsset)
		{
			try
			{
				...
				foreach (var guid in guids)
				{
					...
					// Prefabs need every Object (Components aren't asset representations). For other types we default to
					// representations, which only returns visible sub-assets and never deserializes hidden ones. Hidden sub-assets
					// such as a URP Volume Profile's Volume Components can carry missing scripts whose native deserialization warning
					// can't be suppressed from managed code, so they're skipped unless the user opts in.
					var scanAllObjects = sheetAsset == SheetAsset.Prefab || settings.ScanHiddenSubAssets;
					var subAssets = scanAllObjects ? AssetDatabase.LoadAllAssetsAtPath(path) : AssetDatabase.LoadAllAssetRepresentationsAtPath(path);
					// LoadAllAssetsAtPath includes the main asset so a lone entry means no sub-assets, whereas representations exclude it.
					if (subAssets.Length < (scanAllObjects ? 2 : 1))
					{
						continue;
					}
