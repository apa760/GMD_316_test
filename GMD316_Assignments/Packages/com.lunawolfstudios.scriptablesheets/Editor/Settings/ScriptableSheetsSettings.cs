...
namespace LunaWolfStudiosEditor.ScriptableSheets.Settings
{
	...
	public class ScriptableSheetsSettings : ScriptableSingleton<ScriptableSheetsSettings>
	{
		...
		private bool m_PreviousScanHiddenSubAssets;
		...
		private IndexFormat m_PreviousRowIndexFormat;
		private bool m_PreviousShowColumnIndex;
		private IndexFormat m_PreviousColumnIndexFormat;
		...
		private int m_PreviousCollectionTableDepth;
		...
		public WindowSessionState LoadWindowSessionState(WindowSessionState windowSessionState, HashSet<WindowSessionState> windowSessionStates = null)
		{
			...
			if (windowSessionState.SelectedTypeIndexes == null)
			{
				windowSessionState.SelectedTypeIndexes = new Dictionary<SheetAsset, int>();
			}
		...
		public void DrawGUI(bool isSeparateWindow)
		{
			...
			var hasScanHiddenSubAssetsChanged = m_PreviousScanHiddenSubAssets != m_ObjectManagement.Scan.ScanHiddenSubAssets;
			...
			var hasRowIndexFormatChanged = m_PreviousRowIndexFormat != m_UserInterface.RowIndexFormat;
			var hasShowColumnIndexChanged = m_PreviousShowColumnIndex != m_UserInterface.ShowColumnIndex;
			var hasColumnIndexFormatChanged = m_PreviousColumnIndexFormat != m_UserInterface.ColumnIndexFormat;
			...
			var hasCollectionTablesChanged = m_PreviousCollectionTables != m_UserInterface.CollectionTables;
			...
			var hasManagedReferencesChanged = m_PreviousManagedReferences != m_UserInterface.ManagedReferences;
			...
			var hasCollectionTableDepthChanged = m_PreviousCollectionTableDepth != m_Workload.CollectionTableDepth;

			var needsColumnRefresh = hasScanOptionChanged || hasScanPathChanged
				|| hasHeaderFormatChanged || hasShowRowIndexChanged
				|| hasShowColumnIndexChanged || hasColumnIndexFormatChanged || hasShowChildrenChanged
				|| hasShowArraysChanged || hasOverrideArraySizeChanged || hasArraySizeChanged || hasBestFitArraySizeChanged
				|| hasCollectionTablesChanged || hasShowGuidChanged || hasShowAssetPathChanged
				|| hasShowReadOnlyChanged || hasManagedReferencesChanged || hasMaxIterationsChanged || hasVisibleColumnLimitChanged
				|| hasCollectionTableDepthChanged;
			...
			var needsRepaint = needsColumnRefresh || hasRootPrefabsOnlyChanged || hasScanHiddenSubAssetsChanged || hasCaseSensitiveChanged
				...
				|| hasRowsPerPageChanged || hasSubAssetFiltersChanged || hasRowIndexFormatChanged;
			...
			if (needsRepaint)
			{
				foreach (var window in ScriptableSheetsEditorWindow.Instances)
				{
					...
					else if (hasScanOptionChanged || hasRootPrefabsOnlyChanged || hasScanHiddenSubAssetsChanged)
					{
						window.ScanObjects();
					}
		...
		private void CacheReactiveSettings()
		{
			...
			m_PreviousScanHiddenSubAssets = m_ObjectManagement.Scan.ScanHiddenSubAssets;
			...
			m_PreviousRowIndexFormat = m_UserInterface.RowIndexFormat;
			m_PreviousShowColumnIndex = m_UserInterface.ShowColumnIndex;
			m_PreviousColumnIndexFormat = m_UserInterface.ColumnIndexFormat;
			...
			m_PreviousCollectionTables = m_UserInterface.CollectionTables;
			...
			m_PreviousManagedReferences = m_UserInterface.ManagedReferences;
			...
			m_PreviousCollectionTableDepth = m_Workload.CollectionTableDepth;
		}
