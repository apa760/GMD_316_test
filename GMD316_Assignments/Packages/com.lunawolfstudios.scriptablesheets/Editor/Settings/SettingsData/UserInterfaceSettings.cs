...
namespace LunaWolfStudiosEditor.ScriptableSheets
{
	...
	public class UserInterfaceSettings : AbstractBaseSettings, IScriptableSettings
	{
		...
		[SerializeField]
		private IndexFormat m_RowIndexFormat;
		public IndexFormat RowIndexFormat { get => m_RowIndexFormat; set => m_RowIndexFormat = value; }
		...
		[SerializeField]
		private IndexFormat m_ColumnIndexFormat;
		public IndexFormat ColumnIndexFormat { get => m_ColumnIndexFormat; set => m_ColumnIndexFormat = value; }
		...
		[SerializeField]
		private bool m_CollectionTables;
		public bool CollectionTables { get => m_CollectionTables; set => m_CollectionTables = value; }

		[SerializeField]
		private bool m_ManagedReferences = true;
		public bool ManagedReferences { get => m_ManagedReferences; set => m_ManagedReferences = value; }
		...
		public UserInterfaceSettings()
		{
			...
			m_RowIndexFormat = IndexFormat.Numeric;
			m_ShowColumnIndex = false;
			m_ColumnIndexFormat = IndexFormat.Alpha;
			m_ShowChildren = true;
			m_ShowArrays = true;
			...
			m_CollectionTables = true;
			m_ManagedReferences = true;
		...
		protected override void DrawProperties(SerializedObject target)
		{
			...
			{
				SheetLayout.Indent();
				m_RowIndexFormat = (IndexFormat) EditorGUILayout.EnumPopup(SettingsContent.Dropdown.RowIndexFormat, m_RowIndexFormat);
				SheetLayout.Unindent();
			}
			m_ShowColumnIndex = EditorGUILayout.Toggle(SettingsContent.Toggle.ShowColumnIndex, m_ShowColumnIndex);
			if (m_ShowColumnIndex)
			{
				SheetLayout.Indent();
				m_ColumnIndexFormat = (IndexFormat) EditorGUILayout.EnumPopup(SettingsContent.Dropdown.ColumnIndexFormat, m_ColumnIndexFormat);
				SheetLayout.Unindent();
			}
			...
			m_CollectionTables = EditorGUILayout.Toggle(SettingsContent.Toggle.CollectionTables, m_CollectionTables);
			m_ManagedReferences = EditorGUILayout.Toggle(SettingsContent.Toggle.ManagedReferences, m_ManagedReferences);
